...
#bit PWR_Ctrl = PORTE.3				// Power Control
...
float packages;
static unsigned int16 number_packages;

//////////////////////////////////Package ID Senders////////////////////////////////
static unsigned int16 a,b,n;
static unsigned int16 package_ID;
...
static unsigned int read_from_RAM [1024];
	...
	while(1)
	{
		...
		if(read_data_camera)
		{
			if(write_RAM)
			{
				...
				//if(count_packet == (number_packages - 1)) 			//just for checking
		...
		if(read_RAM)
		{
			for(j=0;j<number_packages;j++)					//can be edit for checking
			{
				...
				if(j == (number_packages - 1))					//can be edit for checking
					read_RAM = 0;
			}
		}
